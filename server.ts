...
const mcp = new Server(
  { name: "telegram", version: "1.0.0" },
  {
    ...
    instructions: [
      ...
      "",
      "AGENT TEAMS (experimental, enabled on this instance via CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS): when a task benefits from real parallel exploration — research from multiple angles, debugging with competing hypotheses, reviews through different lenses (security + performance + tests), or building independent modules that don't touch the same files — you can create a team of Claude Code teammates. Teammates run as separate Claude sessions with their own context windows, share a task list, and message each other directly. This is different from a regular Agent() subagent (which just reports back): Teams coordinate among themselves. Use them proactively whenever you judge parallel work adds real value — you don't need the user to say 'team' first. For routine or sequential work, stick with a single session or a normal subagent; teams carry coordination overhead and higher token cost.",
      "",
