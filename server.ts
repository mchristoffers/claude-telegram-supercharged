...
const mcp = new Server(
  { name: "telegram", version: "1.0.0" },
  {
    ...
    instructions: [
      ...
      "",
      "TWO-TIER MODEL ROUTING: You are running as the fast router (Haiku). Your job is to respond INSTANTLY to simple messages and ESCALATE complex tasks to Opus. Follow this decision tree for EVERY message:",
      "",
      "HANDLE DIRECTLY (you, Haiku — respond in <5 seconds):",
      '- Greetings, thanks, confirmations ("hi", "ok", "thanks", "got it")',
      "- Short factual questions with obvious answers",
      "- Conversation summaries (forwarded message batches)",
      "- Status updates and acknowledgments",
      "- Reactions and emoji responses",
      "- Simple translations or reformatting",
      "- Forwarded conversations — just summarize, don't research",
      "",
      'ESCALATE TO OPUS (use Agent tool with model: "opus"):',
      '- Research tasks ("find out about X", "compare X vs Y", URLs to analyze)',
      "- Code generation or debugging",
      "- Long-form content creation (articles, reports, analyses)",
      "- Multi-step tasks requiring planning",
      "- Complex questions requiring deep reasoning",
      "- Tasks involving web search, fetching URLs, or data analysis",
      "- Anything that needs more than 30 seconds of thinking",
      "",
      'HOW TO ESCALATE: Use the Agent tool: Agent(model: "opus", prompt: "Task description... Reply to Telegram chat_id=XXX via the reply tool with the result."). Pass the chat_id and message_id so the Opus agent can reply directly. IMPORTANT: Before escalating, send a quick Telegram reply like "On it, researching..." so the user knows you received the message. Then spawn the Opus agent.',
      "",
