    ...
    instructions: [
      ...
      ...(IS_HAIKU_ROUTER
        ? [
            ...
            "",
            "HAND TO OPUS (don't do these yourself):",
            "- Web research, URL fetching, or scraping",
            "- Writing code or generating long content",
            "- Anything that would take more than 30 seconds of thinking — STOP and hand off immediately",
            "",
            'HOW TO HAND OFF: (1) Send a quick reply: "On it, researching..." (2) Then spawn a subagent via Agent(model: "opus", prompt: "..."). Include the full task, chat_id, and message_id so Opus can reply directly.',
            "",
            'HAND-OFF PROMPT TEMPLATE: Agent(model: "opus", prompt: "The user in Telegram (chat_id=XXX, message_id=YYY) asked: [TASK]. Research this thoroughly. When done, use the reply MCP tool to send the answer to chat_id=XXX. Also react with 👍 to message_id=YYY.")',
          ]
        : [
            `MODEL ROUTING: You are running on ${ROUTER_MODEL} (1M context). You have a large context window — handle most tasks directly. For extremely heavy reasoning you can spawn an Opus subagent with Agent(model: "opus").`,
            "",
            "RESPOND QUICKLY: Reply to simple messages immediately. For complex tasks, you can handle them yourself — web search, code generation, analysis are all fine. Only spawn an Opus subagent when the task genuinely needs the deepest reasoning (multi-step research, complex code architecture).",
          ]),
      "",
      "AGENT TEAMS (experimental, enabled on this instance via CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS): spawning a team creates multiple independent Claude Code sessions (teammates) with their own context windows, a shared task list, and direct inter-agent messaging — distinct from a regular Agent() subagent which only reports back. Do NOT spawn a team proactively. Create one ONLY when the user explicitly asks for it (e.g. 'spawne ein team', 'create a team', 'use agent teams'). For all other parallel work, use a normal Agent() subagent or handle it yourself. When you do spawn a team, immediately confirm via the reply tool: list each teammate's name and its specific purpose on one line each. Example reply text: 'Team gespawnt:\\n- agent-research: recherchiert X\\n- agent-code: baut Y\\n- agent-review: prüft Z'. This lets the user see what's running and intervene if a teammate is on the wrong track.",
      "",
  ...
  try {
    switch (req.params.name) {
      case "reply": {
        ...
        // Auto-escape MarkdownV2 special chars so Claude doesn't have to
        const finalText = parseMode === "MarkdownV2" ? escapeMarkdownV2(text) : text;
      ...
      case "ask_user": {
        const chat_id = args.chat_id as string;
        const rawAskText = args.text as string;
        const buttons = args.buttons as string[];
        const rawParseMode = (args.parse_mode as string | undefined) ?? "MarkdownV2";
        const parseMode = rawParseMode === "plain" ? undefined : (rawParseMode as "MarkdownV2" | "HTML");
        const text = parseMode === "MarkdownV2" ? escapeMarkdownV2(rawAskText) : rawAskText;
